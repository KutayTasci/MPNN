...
def FakeDataset_Experiment(benchmark, batch_size=128, hidden_channels=64, num_layers=7, learning_rate=0.0001, epochs=10, num_nodes=1000, density=0.1, num_graphs=100):
    num_edges = int(num_nodes * (num_nodes - 1) * density / 2)  # Calculate number of edges based on density
    dataset = Fake_Dataset(num_nodes=num_nodes, num_edges=num_edges, num_graphs=num_graphs)
