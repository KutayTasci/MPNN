...
class DimeNetLayerSum(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr, triplet_info, rbf=None):
        ...
        x_j = self.rbf_mlp(rbf[triplet_info['j_idx']]) if rbf is not None else 0  # [T, D]
        cbf_emb = self.cbf_mlp(triplet_info['cbf'])                    # [T, D]

        # Triplet message: directional info passed to edges (per triplet)
        triplet_msg = cbf_emb + x_k + x_j                              # [T, D]
